    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          ...
          className="px-4 py-2 bg-cloistr-primary text-white rounded hover:bg-cloistr-primary-hover"
        >
          Add Contact
        </button>
      </div>
      ...
      {showForm && (
        <div className="bg-cloistr-bg rounded shadow p-6 mb-6 max-w-md">
          <div className="mb-4">
            ...
            <input
              ...
              className="w-full px-3 py-2 border border-cloistr-border rounded"
            />
          </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {contacts.map((contact: any) => (
          <div
            key={contact.id}
            className="bg-cloistr-bg rounded shadow p-4"
          >
            <h3 className="font-semibold text-cloistr-text">{contact.name}</h3>
            <p className="text-sm text-cloistr-text-muted">{contact.email}</p>
            {contact.npub && (
              <p className="text-xs text-cloistr-text-muted mt-2 truncate">
                {contact.npub}
              </p>
            )}
          </div>
